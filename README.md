...
| `evk4_sdk_advanced/` | Opt-in Metavision SDK Pro layer: seven model-free pipelines (optical flow, tracking, dense flow, spatter, counting, frequency, active-LED) on the event stream (see `docs/sdk/`) |
...
Metavision SDK Pro algorithms (seven model-free pipelines: optical flow,
tracking, dense flow, spatter, counting, frequency, active-LED) on the event
stream. An opt-in layer, not needed for the base pipeline.
