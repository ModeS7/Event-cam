...
| `evk4_sdk_advanced/` | Opt-in Metavision SDK Pro layer: optical flow + object tracking on the event stream (see `docs/sdk/`) |
...
Metavision SDK Pro algorithms (optical flow, object tracking, with more
CV/analytics to come) on the event stream. An opt-in layer, not needed for the
base pipeline.
