...
| `evk4_sdk_advanced/` | Opt-in Metavision SDK Pro layer: sparse optical flow on the event stream (see `docs/sdk/`) |
...
Metavision SDK Pro algorithms (sparse optical flow, with more CV/analytics to
come) on the event stream. An opt-in layer, not needed for the base pipeline.
